...
exports.getArcLoopPath = getArcLoopPath;
 ...
 */
function getLabelPositionStyle(key, placement, autoRotate, offsetX, offsetY, isLoop, loopPlacement) {
    var _a;
    const START_RATIO = 0;
    ...
    if (isLoop && loopPlacement) {
        ...
        offsetX = 0;
        offsetY = 0;
        let angle = (_a = LOOP_TANGENT_ANGLES[loopPlacement]) !== null && _a !== void 0 ? _a : 0;
        ...
        if (shouldRotate)
            transform.push(['rotate', (angle / Math.PI) * 180]);
        return { textAlign: 'center', transform };
    }
 ...
 */
function getArcLoopPath(node, placement, clockwise, dist, sourcePortKey, targetPortKey) {
    const sourcePort = node.getPorts()[(sourcePortKey || targetPortKey)];
    const targetPort = node.getPorts()[(targetPortKey || sourcePortKey)];
    let [sourcePoint, targetPoint] = getLoopEndpoints(node, placement, clockwise, sourcePort, targetPort);
    ...
    const chordMid = [(sourcePoint[0] + targetPoint[0]) / 2, (sourcePoint[1] + targetPoint[1]) / 2, 0];
    const bulgeDirLen = (0, vector_1.distance)(center, chordMid);
    const bulgeDir = bulgeDirLen > 1e-6 ? (0, vector_1.normalize)((0, vector_1.subtract)(chordMid, center)) : [0, -1, 0];
    // 圆弧参数：R = (c² + 4h²) / (8h) | Circular arc parameters
    const chordLen = (0, vector_1.distance)(sourcePoint, targetPoint);
    const h = Math.max(Math.abs(dist), 0.01);
    const R = (chordLen * chordLen + 4 * h * h) / (8 * h);
    // 弧顶点 | Apex point
    const apex = [chordMid[0] + bulgeDir[0] * h, chordMid[1] + bulgeDir[1] * h, 0];
    // 圆弧中心（弧顶点沿凸起方向内缩 R）| Arc center (apex retreated by R along bulge direction)
    const arcCenter = [chordMid[0] + bulgeDir[0] * (h - R), chordMid[1] + bulgeDir[1] * (h - R), 0];
    ...
    if (sourcePort) {
        const srcR = [sourcePoint[0] - arcCenter[0], sourcePoint[1] - arcCenter[1], 0];
        const tangent = sweepFlag === 1 ? [-srcR[1], srcR[0], 0] : [srcR[1], -srcR[0], 0];
        sourcePoint = (0, element_1.getPortConnectionPoint)(sourcePort, (0, vector_1.add)(sourcePoint, tangent));
    }
    if (targetPort) {
        const tgtR = [targetPoint[0] - arcCenter[0], targetPoint[1] - arcCenter[1], 0];
        const tangent = sweepFlag === 1 ? [tgtR[1], -tgtR[0], 0] : [-tgtR[1], tgtR[0], 0];
        targetPoint = (0, element_1.getPortConnectionPoint)(targetPort, (0, vector_1.add)(targetPoint, tangent));
    }
    return [
        ['M', sourcePoint[0], sourcePoint[1]],
